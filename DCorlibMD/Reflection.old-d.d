...
module System.__Internal.Reflection;
...
struct TypeMetadata {
	...
	/// Gets the fields  that this type contains.
	@property ValueMetadata[] Fields() 
	{
...
ValueMetadata findValue(T)(T instance, string name) {
	...
	foreach(ref val; instance.children._values) {
		//std.stdio.writeln("val.name = " ~ val.name);
...
private SymbolContainer getSymbols(alias T)() {
	...
	//std.stdio.writeln("getSymbol:");

	//std.stdio.writeln(statics);
	//enum members = [];// ~ statics;
	...
	//enum mems = statics;
	//static if(statics!=null)
	//static fields are missed by allMember
	//static if(__traits(compiles,T.__staticFieldTuple))
	static if(__traits(compiles,__traits(getMember,T,"__staticFieldTuple")))
	{
		//std.stdio.writeln("compiles and not null");
		//enum val = T.__staticFieldTuple;
		//std.stdio.writeln(T.__statics);
		//static if( __traits(compiles, T.__staticFieldTuple[0]!=&__dummyFunc))
		foreach(m; T.__staticFieldTuple) {
			//std.stdio.writeln(m);
//		static if(m=="__Dummy")
//		{
//			continue;
...
//		}
			...
			ValueMetadata value = getStaticField!(T, m);
			result._values ~= value;
...
private ValueMetadata getStaticField(T, string m)() {
	enum dataKind = DataKind.field;
	//enum index = fieldIndex!(T, m);
	//static if(index == -1)
		//return getField!(BaseClassesTuple!T[0], m);
	//else {
	alias FieldType = typeof(__traits(getMember, T, m));//typeof(T.tupleof[index]);

		TypeInfo type = registerLazyLoader!(typeof(__traits(getMember, T, m)));
		//size_t offset = T.tupleof[index].offsetof; //__traits(getMember, T, m).offsetof;
		DataGetterFunction getter = &getStaticFieldValue!(T, m);
		static if(!isMutable!(FieldType)) {
			// Can't set const or immutable fields:
			DataSetterFunction setter = null;
		} else {
			DataSetterFunction setter = &setStaticFieldValue!(T, m);
		}
		...
		FieldValueMetadata fieldData = FieldValueMetadata(-1, -1, declaringType);
		return ValueMetadata(symbol, dataKind, type, getter, setter, fieldData);
	//}
}
...
private Variant getStaticFieldValue(InstanceType, string m)(ValueMetadata metadata, Variant instanceWrapper) {
	...
	auto result = __traits(getMember, InstanceType, m);//instance.tupleof[fieldIndex];
	return Variant(result);
}

private void setStaticFieldValue(InstanceType, string m)(ValueMetadata metadata, Variant instanceWrapper, Variant valueWrapper) {
	...
	static if(is(InstanceType == const) || is(InstanceType == immutable))
		...
		__traits(getMember, InstanceType, m) = valueWrapper.as!(typeof(__traits(getMember, InstanceType, m)));
}
